...
# Bada damar Mini App UI ya yi magana da Backend
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
# Dauko Keys na Admin daga Railway Environment Variables
ADMIN_GROQ_KEY = os.getenv("GROQ_API_KEY", "PLACEHOLDER_KEY")

# Adana bayanan bots da ke aiki a yanzu
USERS_DB = {} 
RUNNING_BOTS = {} 
...
async def get_ai_reply(text):
    if ADMIN_GROQ_KEY == "PLACEHOLDER_KEY":
        return "Configure GROQ_API_KEY in Railway Variables."
        
    url = "https://api.groq.com/openai/v1/chat/completions"
    headers = {"Authorization": f"Bearer {ADMIN_GROQ_KEY}", "Content-Type": "application/json"}
    
    system_prompt = (
        "You are a regular human participant in a Telegram group chat. "
        "Keep your responses very short, casual, and completely natural. "
        "Never reveal your name, and never reveal your location."
    )
    data = {
...
def create_handler(client, allowed_groups):
...
async def initiate_login(data: UserLoginInit):
    try:
        ...
        return {"status": "success", "message": "OTP sent to Telegram."}
...
async def verify_otp(data: UserOTPVerify):
    ...
    try:
        try:
            ...
            if "Two-step verification" in str(e) or "password" in str(e).lower():
                if not data.password_2fa:
                    return {"status": "requires_2fa", "message": "2FA password required."}
        ...
        return {"status": "success", "message": "Account connected! Wait for Admin activation."}
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def start_user_bot(phone_number):
    ...
    await client.connect()
    create_handler(client, user_data["allowed_groups"])
